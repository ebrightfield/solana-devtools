...
/// which would be found in an IDL.
pub fn ix_discriminator(name: &str) -> AnchorDiscriminator {
...
/// which would be found in an IDL.
pub fn ix_state_discriminator(name: &str) -> AnchorDiscriminator {
    hash(format!("state:{}", name).as_bytes()).to_bytes()[0..8]
        .try_into()
        .unwrap()
}

pub fn partition_discriminator_from_data<const N: usize>(data: &[u8]) -> ([u8; N], &[u8]) {
    let mut first_n_array = [0u8; N];
    let len = data.len().min(N);

    // Copy up to the first 8 bytes into the array
    first_n_array[..len].copy_from_slice(&data[..len]);

    (first_n_array, &data[len..])
}
